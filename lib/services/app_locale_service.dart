...
class AppLocaleService {
  ...
  static final Map<String, Map<String, String>> _translations = {
    ...
    // 學習歷程圖表區塊
    'chart_section_title_matrix': {zhTW: '本週學習歷程 (知識掌握度矩陣)', ja: '今週の学習履歴 (習熟度マトリクス)', ko: '이번 주 학습 이력 (숙련도 매트릭스)'},
    'chart_section_title_radar': {zhTW: '今日學習概況 (測驗狀況)', ja: '今日の学習概要 (テスト状況)', ko: '오늘의 학습 현황 (시험 현황)'},
    ...
    'faq_q8': {zhTW: '如何使用「AI 語音速記」進行 6 大風格整理與轉為文字？', ja: '「AI音声速記」で6大スタイル整理と文字起こしを使うには？', ko: '「AI 음성 속기」로 6가지 스타일 정리 및 텍스트 변환을 사용하는 방법은?'},
    'faq_a8': {zhTW: '在「筆記」頁點擊「+」>「語音錄音速記」，點擊麥克風即可開始收錄。錄畢點擊「轉為文字」，AI 將自動去贅字並標點。您可挑選 6 大風格（課堂重點、會議摘要、結構大綱、代辦結論、精華摘要、日常隨筆），一鍵生成結構化 Markdown、互動心智圖與待辦清單！', ja: '「ノート」の「+」>「音声速記」から録音開始。「文字起こし」をタップするとAIが口語を自動除去し句読点を付与。6つの整理スタイルから選んでマインドマップとノートを即時生成できます！', ko: '「노트」에서 「+」 > 「음성 녹음 속기」를 탭하여 녹음을 시작하세요. 「텍스트 변환」을 누르면 AI가 군더더기 말을 자동 제거하고 문장부호를 정돈합니다. 6가지 스타일을 선택해 마인드맵과 노트를 즉시 생성할 수 있습니다!'},
    'faq_q9': {zhTW: '如何自訂「底部導覽列」的項目順序與抽屜功能？', ja: '「下部ナビゲーションバー」の順序とドロワー機能をカスタマイズするには？', ko: '「하단 바」 항목 순서와 서랍 메뉴 기능을 맞춤 설정하는 방법은?'},
    'faq_a9': {zhTW: '前往「個人檔案」>「自訂導覽列項目順序」，面板中提供 5 格即時動態預覽。您可以直接點擊任兩張項目卡片互換位置，將最常用的 3 項功能放入底欄，第 4 項（如「社群」）收納至側邊抽屜備用，側邊抽屜亦完整保留「社群動態」與各項功能，點擊「儲存套用」立即生效。', ja: '「マイページ」>「ナビゲーションバー並び替え」から5枠のプレビューを確認できます。2つのカードをタップするだけで位置を互換でき、常用する3項目を底面バーに、第4項目（コミュニティ等）をサイドドロワーに収容できます。ドロワーにはタイムライン（動態）も常備されており「保存して適用」で即時反映されます。', ko: '「마이페이지」 > 「하단 바 순서 맞춤설정」에서 5칸 실시간 미리보기를 제공합니다. 두 카드를 탭하여 위치를 직접 맞바꿀 수 있으며, 가장 자주 쓰는 3개 기능을 하단 바에, 4번째 기능(커뮤니티 등)을 사이드 서랍에 보관할 수 있습니다. 서랍에는 피드(동태)도 상시 준비되어 있으며 「저장 및 적용」을 누르면 즉시 반영됩니다.'},
    'faq_q10': {zhTW: 'AI 生成的「互動心智圖」如何縮放與操作？', ja: 'AIが生成した「マインドマップ」の操作方法は？', ko: 'AI가 생성한 「인터랙티브 마인드맵」 조작 방법은?'},
    'faq_a10': {zhTW: '語音速記整理完成後，在預覽介面切換至「心智圖」分頁即可檢視樹狀拓撲。支援雙指縮放手勢、拖曳平移畫布、點擊節點展開/收合子分支，亦可點擊右上角全螢幕按鈕沉浸式複習！', ja: '音声整理後、プレビューの「マインドマップ」タブでツリー構造を確認できます。ピンチズーム、ドラッグ移動、ノードの展開/折りたたみに対応し、全画面表示での復習も可能です！', ko: '음성 정리 후 미리보기의 「마인드맵」 탭에서 트리 구조를 확인할 수 있습니다. 두 손가락 줌, 캔버스 드래그 이동, 노드 탭 접기/펼치기를 지원하며 전체화면 복습도 가능합니다!'},
    'faq_q11': {zhTW: '如何將筆記分享給同學，或一鍵匯入分享碼？', ja: 'ノートを友達と共有したり、共有コードを取り込むには？', ko: '노트를 친구와 공유하거나 공유 코드를 가져오는 방법은?'},
    'faq_a11': {zhTW: '在任何筆記編輯或檢視頁點擊右上角「分享」，系統會產生專屬「Pack 分享碼」。同學只要在筆記列表點選「匯入分享碼」輸入，即可一鍵完整收錄筆記文本、手繪塗鴉與心智圖資料！', ja: 'ノート詳細右上の「共有」から「Pack共有コード」を発行。他のユーザーはノート画面の「共有コードを取り込む」から一発でテキスト・手書き・マインドマップをインポートできます！', ko: '노트 상세 우상단 「공유」에서 「Pack 공유 코드」를 생성할 수 있습니다. 다른 사용자는 노트 화면의 「공유 코드 가져오기」를 통해 텍스트, 손글씨, 마인드맵을 원클릭으로 가져올 수 있습니다!'},
